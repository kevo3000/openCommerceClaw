...
import {
  loadFlowTypes,
  loadFlowTypeDetail,
  registerFlowTypeSchemas,
} from "./views/agents-panel-flowschema.ts";
          ...
          state.tab === "agents"
            ? renderAgents({
                ...
                onSelectPanel: (panel) => {
                  ...
                  if (panel === "flowschema" && state.flowSchema.flowTypes.length === 0) {
                    void loadFlowTypes(
                      (patch) => { state.flowSchema = { ...state.flowSchema, ...patch }; },
                    );
                  }
                },
                ...
                flowSchemaState: state.flowSchema,
                onFlowSchemaSelectType: (id) => {
                  void loadFlowTypeDetail(
                    (patch) => { state.flowSchema = { ...state.flowSchema, ...patch }; },
                    id,
                  );
                },
                onFlowSchemaSwitchView: (v) => {
                  state.flowSchema = { ...state.flowSchema, view: v };
                },
                onFlowSchemaRegister: (id) => {
                  void registerFlowTypeSchemas(
                    (patch) => { state.flowSchema = { ...state.flowSchema, ...patch }; },
                    id,
                  );
                },
